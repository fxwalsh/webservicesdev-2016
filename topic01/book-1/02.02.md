...
Download the sample code supporting the lecture slides (available on Moodle or [here](https://moodle.wit.ie/pluginfile.php/1515450/mod_resource/content/4/samplecode.zip) ) and unzip it. Open a terminal/shell/DOS window, use the _cd_ command to relocate to the folder containing the unzipped files and run the first sample, as follows:
